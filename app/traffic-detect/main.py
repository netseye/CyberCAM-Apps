...
if os.path.exists("./traffic.kmodel"):
    model_path = "./traffic.kmodel"

# 使用系统绝对路径（IDE运行调试）
elif os.path.exists("/data/app/fall-det/traffic.kmodel"):
    model_path = "/data/app/fall-det/traffic.kmodel"

else:
    raise FileNotFoundError("模型文件缺失，请检查当前路径与系统路径下的模型文件是否存在。")

model_size = 640 #模型尺寸
detector = kpu.TRAFFIC_LIGHT_DETECT(model_path, model_size) # 加载模型 
...
def _get_traffic_color(label_index):
    LABEL_COLORS = {
        "red":    (0, 0, 255),    # 红
        "green":  (0, 255, 0),    # 绿
        "yellow": (0, 255, 255),  # 黄
    }
    # 根据标签名称返回固定颜色，让框的颜色跟随红/绿/黄
    name = labels[label_index] if label_index < len(labels) else ""
    return LABEL_COLORS.get(name, (255, 255, 255))
...
while True:
    ...
    for box in boxes:
        ...
        color = _get_traffic_color(box.label)
