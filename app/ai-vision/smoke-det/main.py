...
if os.path.exists("./yolov5s_smoke_best.kmodel"):
    model_path = "./yolov5s_smoke_best.kmodel"

# 使用系统绝对路径（IDE运行调试）
elif os.path.exists("/data/app/fall-det/yolov5s_smoke_best.kmodel"):
    model_path = "/data/app/fall-det/yolov5s_smoke_best.kmodel"

else:
    raise FileNotFoundError("模型文件缺失，请检查当前路径与系统路径下的模型文件是否存在。")

model_size = 640 #模型尺寸
detector = kpu.SMOKE_DETECT(model_path, model_size) # 加载模型 
